...
def parse_args():
   parser = argparse.ArgumentParser("DQN for Atari games, exploration experiment")
   # Environment
   parser.add_argument("--out", type=str, default="defaultText", help="name your experiment")
   parser.add_argument("--explore", type=str, default="random", help="random, greedy, e-greedy, boltzmann, bayesian")
...
class Q_Network():
   def __init__(self):
      ...
      # Boltzmann softmax distribution over (Q_out / temp)
      self.predict = tf.argmax(self.Q_out, 1)
      self.Q_dist = tf.nn.softmax(self.Q_out / self.Temp)

      # Bayesian dropout: Below we obtain the loss by taking the sum of squares difference between the target and prediction Q values.
      ...
      # MODIFY lr from .0005 to .049 (1e^-3)
      trainer = tf.train.GradientDescentOptimizer(learning_rate=0.00005)
...
exploration = args.explore #Exploration method. Choose between: greedy, random, e-greedy, boltzmann, bayesian.
   ...
   for i in range(num_episodes):
      ...
      while j < 999:
         ...
         if total_steps > pre_train_steps and total_steps % 5 == 0:
            ...
            _ = sess.run(q_net.updateModel, feed_dict={q_net.inputs: np.vstack(trainBatch[:, 0]), q_net.nextQ: targetQ,
      ...
      if i % 100 == 0 and i != 0:
         ...
         res = ""
         if exploration == 'e-greedy':
            res = ("Ep. " + str(i) + " Mean Reward: " + str(r_mean) + " Total Steps: " + str(total_steps) + " e: " + str(e))
         if exploration == 'boltzmann':
            res = ("Ep. " + str(i) + " Mean Reward: " + str(r_mean) + " Total Steps: " + str(total_steps) + " t: " + str(e))
         if exploration == 'bayesian':
            res = ("Ep. " + str(i) + " Mean Reward: " + str(r_mean) + " Total Steps: " + str(total_steps) + " p: " + str(e))
         if exploration == 'random' or exploration == 'greedy':
            res = ("Ep. " + str(i) + " Mean Reward: " + str(r_mean) + " Total Steps: " + str(total_steps))
...
#f.write("Percent of succesful episodes: " + str(sum(rList) / num_episodes) + "%")
#f.close()